...
<body>
    ...
    <div class="container">
        <div class="card">
            ...
            {% if tx_msg %}
                <div class="alert {% if 'APPROVED' in tx_msg or 'Active' in tx_msg %}alert-success{% else %}alert-danger{% endif %}">{{ tx_msg }}</div>
            {% endif %}
            
            <form action="/" method="GET" style="display: flex; gap: 10px; margin-bottom: 20px;">
                <input type="text" name="lookup_id" placeholder="Enter Account ID to load..." value="{{ account_id }}" style="flex: 1; padding: 10px; border: 1px solid #ced4da; border-radius: 6px;">
        ...
        <div class="card" style="border-top: 4px solid #198754;">
            ...
            <form action="/onboard" method="POST">
                <div class="input-group">
                    ...
                    <input type="text" name="customer_name" placeholder="e.g., Alice Cooper" required>
                </div>
                <div class="input-group">
...
def index():
    ...
    return render_template_string(HTML_TEMPLATE, account_id=account_id, account_holder="Account Not Found", balance=0.0, hostname=socket.gethostname(), tx_msg="ERROR: Could not query ledger database", onboard_msg=onboard_msg)
...
def transfer():
    ...
    return redirect(url_for("index", lookup_id=source, tx_msg=msg))
...
def onboard():
    ...
    try:
        ...
        if resp.status_code == 201:
            ...
            return redirect(url_for("index", lookup_id=data['account_id'], onboard_msg=msg))
    except Exception as e:
